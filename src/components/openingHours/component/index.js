...
class OpeningHoursController {
  ...
  get isOpen() {
    ...
    return (
      self.hasData() &&
      self.data.current[weekday].hours.length > 0 &&
      self.data.current[weekday].hours[0].open.length > 0
    );
  }
    ...
    if (Primo.record.current.location.item.elementVisible()) {
      ...
      if (location != self.locationCode) {
        ...
        if (response.ok) {
          ...
          Primo.components.get("slsp-i-button-component")[0].style[
            "pointer-events"
          ] = "none";
          Primo.components.get("prm-location-items-after")[1].style["display"] =
            "none";
        } else {
          self.data = {};
        }
